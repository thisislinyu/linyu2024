...
2. 但是我们的数据是observed data, 我们的模型是wrt latent event time的， 所以我们需要一些假设们来得到一些identification.  
...
2. 写出general case的likelihood之后呢，可以看到需要代入 lambda(t) (hazard rate) 和survival function(t), 这就非常的简单～～以pois为例子：lambda(t)  = lambda（constant）；
S(t) 和 hazard function 是有一个公式联系起来的，S(T) = exp(-lambda(t)*T).带进去就行。
